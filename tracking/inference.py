...
def constructBayesNet(gameState: hunters.GameState):
    ...
    #following instructions and assigning vars
    ...
    #settings constants all possible x and y values 
    ...
    #constructing bayes net with values 
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        #setting conditional prob table with conditioned varaiables to iterate through...
        probTables = bayesNet.getAllCPTsWithEvidence(evidenceDict) 

        for i in eliminationOrder: 
            probTables, value = joinFactorsByVariable(probTables,i)
            if len(value.unconditionedVariables())!=1: 
                valToEliminate = eliminate(value,i)
                probTables.append(valToEliminate) #add var to prob table

        # normalize so that the probability sums to one
        eviVar = normalize(joinFactors(probTables))
        return eviVar
